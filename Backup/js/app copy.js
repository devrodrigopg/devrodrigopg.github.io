...
XReader.js Version ${xreader.Environment.AppVersion}`,
      ...
      // init renditions
      var renditions = isMobile()
        ? xreader.Document.Renditions.Mobile
        : xreader.Document.Renditions.Desktop;
      app.renditions = renditions;

      // init UI
      $("body").css(
        "background-color",
        xreader.Document.Settings.backgroundColor
      );
      ...
      $("#nextPageBtn").click(function (e) {
        e.preventDefault();
        !xreader.isTurningPages && app.goToNextSpread();
      });
      $("#previousPageBtn").click(function (e) {
        e.preventDefault();
        !xreader.isTurningPages && app.goToPreviousSpread();
      });
      ...
      $("#readaloudToggleBtn").on(
        app.events.readaloudPlaybackStatusChanged,
        function () {
          $(this).html(
            !app.readaloudIsPlaying ? "Start Read Aloud" : "Stop Read Aloud"
          );
        }
      ); // localize
      $("#readaloudTurnPagesCheckbox").on(
        app.events.readaloudPlaybackStatusChanged,
        function () {
          if (this.checked != app.readaloudTurnsPagesAutomatically)
            this.checked = app.readaloudTurnsPagesAutomatically;
        }
      );
      $("#soundtrackToggleBtn").on(
        app.events.soundtrackPlaybackStatusChanged,
        function () {
          $(this).html(
            !app.soundtrackIsPlaying ? "Start Soundtrack" : "Stop Soundtrack"
          );
        }
      ); // localize
      $("#searchControlsModal").on("show.bs.modal", app.initSearch);
      $("#searchControlsModal").on("shown.bs.modal", () => {
        $("#searchField").focus();
      });
      ...
      if (app.renditions.localizations.length > 1) {
        app.renditions.localizations.forEach((localization) => {
          var el =
            '<a href="#" class="list-group-item list-group-item-action" data-xr-language-code="' +
        ...
        $("#languageSelector .list-group-item").click(function () {
          app.localStorage.setItem(
            ...
            $(this).attr("data-xr-language-code")
          );
          app.updateReader(true);
        });
      } else {
      ...
      if (getCookie("lms_assessments_enabled") === "true") {
        xreader.Hooks.BeforePageLoad = function (pageSrc) {
          xreader.executeJavascript(pageSrc, function ($, window, document) {
            var disableAutoInitSelectors = window.pubcoder
              .disableAutoInitSelectors || [
              "SCQuiz",
              "SCCrosswords",
              "SCFillInTheGaps",
              "SCTrueFalse",
              "SCMatchWordWithPicture",
            ];
            disableAutoInitSelectors.forEach(function (selector) {
              var elements = $("." + selector);
              console.warn(
                "disabling auto-init for " + elements.length + " " + selector
              );
    ...
    "XPUB.Ready": function (params) {
      ...
      if (
        app.readaloudIsPlaying &&
        pageUrl == app.rendition.contentsUrl + app.currentPage.url
      ) {
        app.startReadaloud(app.currentPage);
      }
      if (app.currentSearchPattern.length > 0) {
        xreader.executeJavascript("*", function ($, window, document) {
          window.XPUB.find(app.currentSearchPattern);
        });
      }
    },
    ...
    "XPUB.OpenUrl": function (params) {
      ...
      if (
        url.substring(0, app.rendition.contentsUrl.length) ==
        app.rendition.contentsUrl
      ) {
        var pageUrl = url.substring(app.rendition.contentsUrl.length);
        var destPage = _(app.rendition.data.pages).findWhere({ url: pageUrl });
        if (destPage != null)
          app.goToPage(app.rendition.data.pages.indexOf(destPage) + 1);
    ...
    "XPUB.PlayAudio": function (params) {
      ...
      if (buffer == null) {
        ...
        request.onload = function () {
          app.audioContext.decodeAudioData(
            request.response,
            function (buffer) {
        ...
        source.onended = function () {
          ...
          try {
            xreader
              .getPageContentWindow(senderPageUrl)
              .$("#" + objectIdInPage)
              .trigger("ended");
          } catch (error) {}
        };
    ...
    "XPUB.SeekReadaloud": function (params) {
      ...
      if (
        page == null ||
        app.rendition.contentsUrl + page.url != senderPageUrl
      ) {
        var url = senderPageUrl.substr(app.rendition.contentsUrl.length);
        // console.warn(url);
        page = _(app.rendition.data.pages).find(function (p) {
          return p.url == url;
        });
        if (page == null) return;
      ...
      var block = _(page.readaloudBlocks).find(function (b) {
        return b.DOMElementId == blockId;
      });
    ...
    "XPUB.StartStopReadAloud": function (params) {
      if (app.readaloudIsPlaying) {
        ...
        var senderPageUrl = params[0];
        var url = senderPageUrl.substr(app.rendition.contentsUrl.length);
        var page = _(app.rendition.data.pages).find(function (p) {
          return p.url == url;
        });
        app.startReadaloud(page);
      }
  ...
  startReadaloud: function (page, startFromBeginning = false) {
    ...
    if (page.readaloudAudioUrl == null && app.isFacingPages) {
      ...
      if (
        app.currentPageNumber % 2 == 0 &&
        app.rendition.data.pages.length > app.currentPageNumber &&
        app.rendition.data.pages[app.currentPageNumber].readaloudAudioUrl !=
          null
      ) {
        app.goToNextPage();
        app.startReadaloud(app.currentPage);
        ...
        app.currentPageNumber % 2 == 1 &&
        app.currentPageNumber > 1 &&
        app.rendition.data.pages[app.currentPageNumber - 2].readaloudAudioUrl !=
          null
      ) {
    ...
    xreader.applyMediaOverlay(
      self.readaloudIsPlaying,
      app.rendition.contentsUrl + page.url,
      self.readaloudCurrentBlock != null
        ? self.readaloudCurrentBlock.DOMElementId
        : null,
      !player.paused
    );
    ...
    if (self.readaloudInterval == null) {
      self.readaloudInterval = setInterval(function () {
        ...
        if (page.readaloudBlocks == null) return;
        var block = _(page.readaloudBlocks).find(function (b) {
          return time >= b.ClipBegin && time < b.ClipEnd;
        });
        if (block != self.readaloudCurrentBlock) {
          ...
          xreader.applyMediaOverlay(
            self.readaloudIsPlaying,
            app.rendition.contentsUrl + page.url,
            block != null ? block.DOMElementId : null,
            !player.paused
          );
        }
      }, 50);
  ...
  onPageReadaloudEnded: function () {
    ...
    xreader.executeJavascript("*", ($) =>
      $(".SCPageObject").trigger("SCEventReadAloudPageCompleted")
    );

    // eventually go next page and start its media overlay
    if (
      self.isFacingPages &&
      self.currentPageNumber % 2 == 0 &&
      self.rendition.data.pages.length > self.currentPageNumber
    ) {
      self.goToNextPage();
          ...
          .each(function () {
            try {
              ...
              readaloudBlocks.push({
                ...
                ClipBegin: smilTimeToSeconds(
                  $(this).find("audio").attr("clipBegin")
                ),
                ClipEnd: smilTimeToSeconds(
                  $(this).find("audio").attr("clipEnd")
                ),
              });
            } catch (err) {
              throw err;
  ...
  scrollPageSwitcherToActivePage: function (animated) {
    ...
    if (
      toffset < 0 ||
      toffset + $(".pageThumbnail.active").width() >
        $(".pagesThumbnailsContainer").width()
    ) {
  ...
  updateReader: function (forceUpdate) {
    ...
    if (shouldSelectRenditionFromUrl) {
      ...
      try {
        ...
        if (queryParams.get("language") != null) {
          var foundRendition = _(app.renditions.localizations).findWhere({
            languageCode: queryParams.get("language"),
          });
          if (foundRendition != null) {
            console.log(
              "forcing " +
                foundRendition.languageCode +
                " rendition from URL parameters"
            );
    ...
    if (preferredLanguageCode == null) {
      preferredLanguageCode =
        app.localStorage.getItem("userPreferredLanguageCode") ||
        navigator.language; // e.g. en-US
    }

    // find the best rendition matching current settings
    var rendition = app.getBestRenditionMatching(
      width,
      height,
      preferredLanguageCode
    );
    ...
    app.isFacingPages =
      xreader.Document.Settings.facingPagesInLandscape &&
      width > height &&
      rendition.width < rendition.height;

    var previousRendition = app.rendition;
    var previousPageId =
      app.currentPageNumber > 0 && app.rendition != null
        ? app.rendition.data.pages[app.currentPageNumber - 1].id
        : null;
    ...
    app.loadRendition(rendition, function () {
      if (!forceUpdate) {
        if (
          app.rendition == previousRendition &&
          app.isFacingPages == previousFacingPages
        ) {
      ...
      app.isRightToLeft =
        app.rendition.data.meta["page-progression-direction"] == "rtl";
      ...
      if (
        app.rendition.data.options.enableReadaloud ||
        app.rendition.data.options.enableSoundtrack
      ) {
        $("#audioControlsBtn").show();
        $("#readaloudSettingsContainer").css(
          "display",
          app.rendition.data.options.enableReadaloud ? "block" : "none"
        );
        $("#soundtrackSettingsContainer").css(
          "display",
          app.rendition.data.options.enableSoundtrack ? "block" : "none"
        );
      } else {
      ...
      $(
        "#languageSelector .list-group-item[data-xr-language-code='" +
          app.rendition.languageCode +
          "']"
      ).addClass("active");
      ...
      if (rendition.data.toc.length > 1) {
        ...
        rendition.data.toc.forEach((chapter) => {
          var el =
            '<a href="#" class="list-group-item list-group-item-action" data-xr-page-number="' +
            chapter.pageNumber +
            '">' +
            chapter.title +
            "</a>";
      ...
      rendition.data.pages.forEach((page) => {
        var el =
          '<a href="#" class="pageThumbnail" data-xr-page-number="' +
          page.number +
          '"><img class="b-lazy" src="' +
          rendition.contentsUrl +
          'images/default-thumbnail.png" data-src="' +
          rendition.contentsUrl +
      ...
      $("#navigation").css(
        "display",
        rendition.data.options.navigationArrows ? "block" : "none"
      );
      ...
      if (previousPageId != null) {
        ...
        pageToNavigate = _(app.rendition.data.pages).findWhere({
          id: previousPageId,
        });
      }
      ...
      if (pageToNavigate == null) {
        try {
          ...
          if (queryParams.get("startPage") != null) {
            pageToNavigate = _(app.rendition.data.pages).findWhere({
              number: parseInt(queryParams.get("startPage")),
            });
          }
        } catch (err) {
          console.error(err);
      ...
      if (
        pageToNavigate == null &&
        xreader.Document.Settings.restoreLatestPage
      ) {
        // not implemented yet
      ...
      var destPage =
        pageToNavigate != null
          ? app.rendition.data.pages.indexOf(pageToNavigate) + 1
          : 1;
  ...
  getBestRenditionMatching: function (width, height, preferredLanguageCode) {
    ...
    var results = _(app.renditions.list).where({
    ...
    if (results.length == 0 && preferredLanguageCode.indexOf("-") >= 0) {
      ...
      results = _(app.renditions.list).where({
        languageCode: preferredLanguageCode,
      });
    }
    ...
    if (results.length == 0)
      results = _(app.renditions.list).where({ isPreferred: true });
    ...
    var ratio = _(results).reduce(function (bestMatch, rendition) {
      if (bestMatch == null) {
        ...
        return Math.abs(rendition.aspectRatio - currentRatio) <
          Math.abs(bestMatch.aspectRatio - currentRatio)
          ? rendition
          : bestMatch;
      }
    }).aspectRatio;
    results = _(results).filter(function (r) {
      return Math.abs(r.aspectRatio - ratio) < 0.01;
    });

    // select renditions with resoultion equal or higher than current resolution
    var currentResolution = width * height;
    var higherResResults = _(results).filter(function (r) {
      return r.width * r.height >= currentResolution;
    });
      ...
      return _(higherResResults).reduce(function (bestMatch, rendition) {
        if (bestMatch == null) {
          ...
          return rendition.width * rendition.height <
            bestMatch.width * bestMatch.height
            ? rendition
            : bestMatch;
        }
      });
  ...
  loadRendition: function (rendition, callback) {
    ...
    myScript.setAttribute(
      "src",
      rendition.baseUrl + "META-INF/com.pubcoder.xreader.meta.js"
    );
  ...
  goToPage: function (pageNumber, forceReload) {
    ...
    if (pageNumber > app.rendition.data.pages.length)
      pageNumber = app.rendition.data.pages.length;
    ...
    if (app.currentPageNumber > 0) {
      ...
      if (
        useDoublePage &&
        _(visiblePages).contains(pageNumber) &&
      ...
      $(
        ".pageThumbnail[data-xr-page-number='" + app.currentPageNumber + "']"
      ).removeClass("active");
    }
    ...
    var startIndex =
      pageNumber - (useDoublePage && pageNumber % 2 != 0 ? 2 : 1);
    var index = startIndex;
    var baseUrl = app.rendition.contentsUrl;

    var pageOne =
      index >= 0 ? baseUrl + app.rendition.data.pages[index].url : "";
    var pageObjectOne =
      pageOne.length > 0 ? app.rendition.data.pages[index] : null;
    ...
    var pageTwo =
      useDoublePage && ++index < app.rendition.data.pages.length
        ? baseUrl + app.rendition.data.pages[index].url
        : "";
    var pageObjectTwo =
      pageTwo.length > 0 ? app.rendition.data.pages[index] : null;
    ...
    var thumbRightOne =
      ++index < app.rendition.data.pages.length
        ? baseUrl + app.rendition.data.pages[index].loadingImageUrl
        : "";
    ...
    var thumbLeftTwo =
      useDoublePage && --index >= 0
        ? baseUrl + app.rendition.data.pages[index].loadingImageUrl
        : "";

    if (useDoublePage) {
      var tmp = thumbLeftTwo;
      thumbLeftTwo = thumbLeftOne;
      thumbLeftOne = tmp;
    }
    ...
    if (isRightToLeft) {
      if (useDoublePage) {
        ...
        thumbOne = tmp;
        tmp = thumbRightOne;
        thumbRightOne = thumbLeftTwo;
        thumbLeftTwo = tmp;
        tmp = thumbRightTwo;
        thumbRightTwo = thumbLeftOne;
        thumbLeftOne = tmp;
      } else {
        var tmp = thumbRightOne;
        thumbRightOne = thumbLeftOne;
        thumbLeftOne = tmp;
      }
    }
    ...
    var goToPageFn = function () {
      xreader.load(
        readerSettings,
        app.rendition.data.meta.width,
        app.rendition.data.meta.height,
        pageOne,
        pageTwo,
        thumbOne,
        thumbTwo,
      ...
      if (isRightToLeft) {
        xreader.canDragPageRight =
          endIndex < app.rendition.data.pages.length - 1;
  ...
  goToPageAtRight: function () {
    ...
    if (destPage > app.rendition.data.pages.length)
      destPage = app.rendition.data.pages.length;
  ...
  getVisiblePages: function () {
    ...
    visiblePages = _(visiblePages).reject(function (x) {
      return x < 1 || x > app.rendition.data.pages.length;
    });
    leftPages = _(leftPages).reject(function (x) {
      return x < 1 || x > app.rendition.data.pages.length;
    });
    rightPages = _(rightPages).reject(function (x) {
      return x < 1 || x > app.rendition.data.pages.length;
    });
    ...
    if (pattern.length > 0) {
      ...
      _(rendition.indexedText).each((txt, index) => {
        ...
        if (found) {
          ...
          var el =
            '<a href="#" class="searchResultPageThumbnail" data-xr-page-number="' +
            page.number +
            '"><img class="b-lazy" src="' +
            rendition.contentsUrl +
            'images/default-thumbnail.png" data-src="' +
  ...
  searchInPage: function () {
    if (app.currentSearchPattern.length > 0) {
      ...
      xreader.executeJavascript("*", function ($, window, document) {
        try {
          window.XPUB.clearSearchResults();
        } catch (err) {}
      });
    }
  },
