...
var xreader = window.PubViewer || {
  ...
  init: function () {
    ...
    if (
      ...
      xreader.Environment.Idiom == xreader.Idiom.Desktop
    ) {
    ...
    window.addEventListener("message", function (event) {
      ...
      try {
        ...
        document
          .getElementById("ReaderPageOne")
          .contentWindow.postMessage(message, "*");
        document
          .getElementById("ReaderPageTwo")
          .contentWindow.postMessage(message, "*");
      } catch (error) {
        console.error(error);
      }
  ...
  onWindowResize: function () {
    ...
    if (
      (window.innerWidth > window.innerHeight &&
        xreader.containerWidth > xreader.containerHeight) ||
      (window.innerWidth < window.innerHeight &&
        xreader.containerWidth < xreader.containerHeight)
    ) {
    ...
    xreader.callAppMethod("WindowResize", [
      window.innerWidth,
      window.innerHeight,
    ]);
  },
  ...
  handleReaderTouchAction: function (e) {
    ...
    if (
      Math.abs(xreader.lastReaderTouchEndTime - xreader.lastReaderTapTime) < 250
    ) {
  ...
  onTap: function (ev, iframeEvent) {
    ...
    if (
      e.defaultPrevented ||
      !eventComesFromRealClick ||
      xreader.wasDraggingPage ||
      (e.target != null &&
        neverShowMenuObjects.indexOf(e.target.localName) >= 0)
    ) {
    ...
    if (
      e.target != null &&
      neverSwipeObjects.indexOf(e.target.localName) >= 0 &&
      e.target.controls
    ) {
      // console.log("canceled click");
      xreader.wasDraggingPage = false;
  ...
  onKeyDown: function (ev, iframeEvent) {
    ...
    if (
      xreader.ReaderSettings.EnableSwipeToNavigate ||
      xreader.ReaderSettings.EnableNavigationArrows
    ) {
      if (e.keyCode == 39) {
        // arrow right
  ...
  onTouchStart: function (ev, iframeEvent) {
    ...
    if (e.target != null) {
      ...
      while (
        el != null &&
        !el.classList.contains("SCContent") &&
        !el.classList.contains("SCOverlay") &&
        !el.classList.contains("SCPage") &&
        el.nodeName != "body"
      ) {
        // console.log(el);
  ...
  onTouchMove: function (ev, iframeEvent) {
    ...
    if (e.touches.length == 1 && xreader.canDragPage) {
      xreader.touchMoves++;
      var multiplier =
        xreader.Environment.Platform == xreader.Platform.Android
          ? xreader.containerHeight / screen.height
          : 1.0;
      ...
      if (!(e.ownerDocument == document && e.target.id == "overlay"))
        e.preventDefault(); // preventing default will make vertical scrolling not work / not preventing it will make event fire only once on Android 4.4
      xreader.draggingPage = true;

      // try to load the loading images before they are actually used, to avoid flickering
      // if (xreader.Environment.Platform != xreader.Platform.Android) {
      var prepareWrapperOne =
        $("#ReaderPageOne").css("opacity") == "1" &&
        $("#ReaderPageOne").css("display") == "block"; // otherwise previous page is still loading and wrapper background is visible!!
      var prepareWrapperTwo =
        $("#ReaderPageTwo").css("opacity") == "1" &&
        $("#ReaderPageTwo").css("display") == "block"; // otherwise previous page is still loading and wrapper background is visible!!
      if (xreader.currentDragX <= 0 && dx > 0) {
        if (prepareWrapperOne)
          document.getElementById(
            "ReaderPageOneWrapper"
          ).style.backgroundImage = document.getElementById(
            "ReaderLeftThumbnailOne"
          ).style.backgroundImage;
        if (prepareWrapperTwo)
          document.getElementById(
            "ReaderPageTwoWrapper"
          ).style.backgroundImage = document.getElementById(
            "ReaderLeftThumbnailTwo"
          ).style.backgroundImage;
        ...
        if (prepareWrapperOne)
          document.getElementById(
            ...
            "ReaderRightThumbnailOne"
          ).style.backgroundImage;
        if (prepareWrapperTwo)
          document.getElementById(
            "ReaderPageTwoWrapper"
          ).style.backgroundImage = document.getElementById(
            "ReaderRightThumbnailTwo"
          ).style.backgroundImage;
      }
      // }
  ...
  onTouchEnd: function (ev, iframeEvent) {
    ...
    var speed =
      Math.abs(xreader.currentDragX) / xreader.containerWidth / (ms / 1000);
    ...
    if (
      !xreader.touchActionDefaultPrevented &&
      Date.now() - xreader.touchStartTime < 250
    ) {
      xreader.handleReaderTouchAction(e);
    }
  },
  ...
  onWheel: function (ev, iframeEvent) {
    ...
    if (
      ...
      // ignore slow or inertial scrolling
      xreader.accWheelX = 0;
      xreader.accWheelY = 0;
      return;
    }
    ...
    if (
      Math.abs(xreader.accWheelX) > 500 &&
      Math.abs(xreader.accWheelY) < 100
    ) {
  ...
  adjustInterface: function (customWidth, customHeight) {
    ...
    var leftThumbnailOneColumn = document.getElementById(
      "ReaderLeftThumbnailOneColumn"
    );
    var leftThumbnailTwoColumn = document.getElementById(
    ...
    var rightThumbnailOneColumn = document.getElementById(
      "ReaderRightThumbnailOneColumn"
    );
    var rightThumbnailTwoColumn = document.getElementById(
    ...
    var cw = Math.round(
      xreader.containerWidth / (xreader.isDoublePage ? 2 : 1)
    );
    ...
    $(".ReaderPageWrapper").css(
      "top",
      Math.round((ch - wrapperHeight) / 2) + "px"
    );
  ...
  load: function (
    ...
    if (!readerSettings.EnablePinchToZoom)
      metaViewportContent += ", user-scalable=no";
    if (readerSettings.ViewportFitCover)
      metaViewportContent += ", viewport-fit=cover";
    document
    ...
    if (xreader.Environment.Platform == xreader.Platform.iOS) {
      xreader.adjustInterface(
        readerSettings.WebViewWidth,
        readerSettings.WebViewHeight
      );
    } else {
      xreader.adjustInterface();
    }
    ...
    if (pageOne == null || pageOne == "") {
      ...
      setTimeout(function () {
        if (
          $(pageOneContainer).css("display") != "none" &&
          $(pageOneContainer).css("opacity") != "1" &&
    ...
    if (pageTwo == null || pageTwo == "") {
      ...
      setTimeout(function () {
        if (
          $(pageTwoContainer).css("display") != "none" &&
          $(pageTwoContainer).css("opacity") != "1" &&
    ...
    setTimeout(function () {
      document.getElementById("ReaderLeftThumbnailOne").style.backgroundImage =
        "url('" + thumbLeftOne + "')";
      document.getElementById("ReaderLeftThumbnailTwo").style.backgroundImage =
        "url('" + thumbLeftTwo + "')";
      document.getElementById("ReaderRightThumbnailOne").style.backgroundImage =
        "url('" + thumbRightOne + "')";
      document.getElementById("ReaderRightThumbnailTwo").style.backgroundImage =
        "url('" + thumbRightTwo + "')";
      xreader.initReaderPage(
        pageOneContainer,
        pageOne,
        xreader.pageInstanceID++
      );
    }, 25);
    setTimeout(function () {
      xreader.initReaderPage(
  ...
  initReaderPage: function (iframeElement, src, pagePageInstanceId) {
    ...
    if (
      ...
      iframeElement.contentWindow.document.body.getAttribute(
        "XReaderPagePageInstanceId"
      ) != null &&
      iframeElement.contentWindow.document.body.getAttribute(
        "XReaderPagePageInstanceId"
      ) > pagePageInstanceId
    ...
    if (
      ...
      setTimeout(function () {
        xreader.initReaderPage(iframeElement, src, pagePageInstanceId);
      }, 100);
      return;
    }
    ...
    $.each(pageIframes, function (index, item) {
      item.onload = function () {
        ...
        var internalIframeInjectedCss =
          "* { -webkit-tap-highlight-color: transparent; }";
        var internalIframeInjectedStyleNode =
          internalIframeDocument.createElement("style");
        internalIframeInjectedStyleNode.innerHTML = internalIframeInjectedCss;
        internalIframeDocument.head.appendChild(
          internalIframeInjectedStyleNode
        );
      };
    });
    ...
    if (xreader.ReaderSettings.EnableTapToSeekReadaloud) {
      $(iframeBody.getElementsByClassName("SCReadaloudBlock")).click(function (
        e
      ) {
    ...
    var injectedStyleNode = iframeDocument.createElement("style");
    injectedStyleNode.innerHTML = injectedCss;
    iframeDocument.head.appendChild(injectedStyleNode);

    // define current orientation
    if (xreader.orientation != undefined)
      xreader.setOrientation(xreader.orientation);
  ...
  turnPagesRightAnimated: function (onCompletionFunction) {
    ...
    if (onCompletionFunction == null) {
      ...
      document.getElementById("ReaderPageOneWrapper").style.backgroundImage =
        document.getElementById(
          "ReaderRightThumbnailOne"
        ).style.backgroundImage;
      document.getElementById("ReaderPageTwoWrapper").style.backgroundImage =
        document.getElementById(
          "ReaderRightThumbnailTwo"
        ).style.backgroundImage;
    }
    $("#container").transition(
      { x: -xreader.containerWidth },
      150,
      "out",
      onCompletionFunction
    );
  ...
  applyMediaOverlay: function (
    ...
    var iframeOne = document.getElementById("ReaderPageOne");
    var iframeTwo = document.getElementById("ReaderPageTwo");
    var eventToTrigger = enabled
      ? "SCEventReadAloudStarted"
      : "SCEventReadAloudStopped";

    [iframeOne, iframeTwo].forEach(function (iframe) {
      try {
        $(
          iframe.contentWindow.document.getElementsByClassName(
            xreader.MediaOverlay.ActiveElementClass
          )
        ).removeClass(xreader.MediaOverlay.ActiveElementClass);
    ...
    $(pageContentWindow.document.getElementById(elementId)).addClass(
      xreader.MediaOverlay.ActiveElementClass
    );
  },
  ...
  executeJavascript: function (pageSrc, fn) {
    if (pageSrc == "*") {
      ...
      pageContentWindow.tmpExecJsFn(
        pageContentWindow.$,
        pageContentWindow,
        pageContentWindow.document
      );
    }
  },
  ...
  backupUserData: function (indexedDBs) {
    ...
    if (
      idbCount > 0 &&
      xreader.Environment.Platform != xreader.Platform.Windows
    ) {
      var checkCompleted = function () {
        ...
        if (idbBackedUp == idbCount && idbStoresCount == idbStoresBackedUp) {
          ...
          console.log(idbData);
          xreader.callAppMethod("BackupUserData", [
            localStorageData,
            indexedDbData,
            nativeStorageData,
          ]);
  ...
  restoreUserData: function (
    ...
    // restore indexed db
    if (
      indexedDbBackup != null &&
      xreader.Environment.Platform != xreader.Platform.Windows
    ) {
      indexedDbBackup.forEach((dbBackup) => {
        try {
          var restoreDatabase = function () {
            indexedDB.open(dbBackup.name, dbBackup.version).onupgradeneeded =
              function (event) {
                var db = event.target.result;
                dbBackup.objectStores.forEach((objectStoreBackup) => {
                  var objectStore = db.createObjectStore(
                    objectStoreBackup.name,
                    {
                      keyPath: objectStoreBackup.keyPath,
                      autoIncrement: objectStoreBackup.autoIncrement,
                    }
                  );
                  objectStore.transaction.oncomplete = function (event) {
                    var objectStore = db
                      .transaction(objectStoreBackup.name, "readwrite")
                      .objectStore(objectStoreBackup.name);
...
function XPUB(iframeElement) {
  ...
  this.readerPrefersToHandleAudio =
    xreader.Environment.Platform == xreader.Platform.iOS ||
    (xreader.Environment.Platform == xreader.Platform.Web &&
 ...
 */
XPUB.prototype.callAppMethod = function (
  methodName,
  methodParameters,
  callbackFn,
  errorFn
...
XPUB.prototype.ready = function () {
  var caller = this;
  setTimeout(function () {
    if (caller.iframeElement.contentWindow.location.href != caller.pageUrl)
      return; // frame loaded another page in the meantime
...
XPUB.prototype.resumeAllMedia = function () {
  ...
  $(doc)
    .find("[data-mediaPausedByXPUB='true']")
    .each(function (index) {
...
XPUB.prototype.getOrientation = function () {
  return xreader.overriddenOrientation != null
    ? xreader.overriddenOrientation
    : window.orientation;
};
